...
1. Abra o **PowerShell como Administrador**:
   ...
   - Clique **Sim** na janela que aparecer

2. Cole este comando e pressione **Enter**:

```powershell
Set-ExecutionPolicy Bypass -Scope Process -Force; irm https://raw.githubusercontent.com/filipegajo89/anki-tec/main/public/install_windows.ps1 | iex
```

3. Siga as instruções na tela (leva ~15 minutos)

> 💡 **Nunca usou o PowerShell?** Não se preocupe! Você só precisa colar o comando acima. O "Executar como administrador" é necessário para instalar os programas.

> ⚠️ Se o comando não funcionar, pode ser que o `winget` não esteja instalado. Nesse caso, o script tentará abrir a Microsoft Store para você instalar o "Instalador de Aplicativo".

---

## 📋 O que o script instala automaticamente

| Programa | Para que serve | Instalado via |
|----------|---------------|---------------|
| **Google Chrome** | Navegador onde o script funciona | winget |
| **Anki** | Programa de flashcards com repetição espaçada | winget |
| **Obsidian** | Programa para organizar suas notas de estudo | winget |
| **Node.js** | Necessário para o MCP filesystem | winget |
| **uv** | Gerenciador Python moderno | instalador oficial |
| **notebooklm-mcp-cli** | CLI para gerar podcasts via NotebookLM | uv |
...
4. O áudio será salvo em **Documentos → Podcasts**
...
| "winget não encontrado" | Instale o "Instalador de Aplicativo" da Microsoft Store |
| "Acesso negado" | Execute o PowerShell como Administrador |
| "nlm login falhou" | Abra o PowerShell e rode `nlm login` novamente |
| "Execução de scripts desabilitada" | Execute: `Set-ExecutionPolicy Bypass -Scope Process -Force` antes do comando |
...
Para atualizar o notebooklm-mcp-cli, abra o PowerShell e rode:
```powershell
...
| Podcasts | Documentos → pasta `Podcasts/` |
| Config Claude | `%APPDATA%\Claude\claude_desktop_config.json` |
| Config Anki | `%APPDATA%\Anki2\` |
...
**P: Posso usar no Edge ou Firefox?**
R: Recomendamos o Chrome. O Tampermonkey funciona no Edge (com Chromium), mas não testamos.

**P: O script funciona em cadernos que já fiz?**
R: Sim! Basta voltar ao caderno no TEC Concursos.

**P: Preciso do Windows 10 ou 11?**
R: Windows 10 (versão 1809+) ou Windows 11. Versões mais antigas não têm winget.

**P: Como desinstalar tudo?**
R: No PowerShell (como Admin), rode:
```powershell
